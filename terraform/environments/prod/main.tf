...
module "rds" {
  ...
  replica_kms_key_arn        = var.dr_kms_key_arn
  tags                       = local.common_tags
}
...
# Client VPN — admin access (cert + MFA, internal ALB only)
module "vpn" {
  source                      = "../../modules/vpn"
  name                        = local.name
  vpc_id                      = module.vpc.vpc_id
...
# Allow VPN clients (10.8.0.0/22) to reach EKS nodes through the internal ALB
...
# Route 53 latency routing (EU + US)
...
module "irsa_catalog" {
  ...
  inline_policy    = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Effect = "Allow"; Action = ["secretsmanager:GetSecretValue"]; Resource = "arn:aws:secretsmanager:${var.aws_region}:*:secret:shopcloud/prod/catalog*" },
      { Effect = "Allow"; Action = ["ssm:GetParameter"]; Resource = "arn:aws:ssm:${var.aws_region}:*:parameter/shopcloud/prod/*" },
...
module "irsa_checkout" {
  ...
  inline_policy    = jsonencode({
    ...
    Statement = [
      ...
      { Effect = "Allow"; Action = ["secretsmanager:GetSecretValue"]; Resource = "arn:aws:secretsmanager:${var.aws_region}:*:secret:shopcloud/prod/checkout*" },
...
module "irsa_cart" {
  ...
  inline_policy     = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Effect = "Allow"; Action = ["secretsmanager:GetSecretValue"]; Resource = "arn:aws:secretsmanager:${var.aws_region}:*:secret:shopcloud/prod/cart*" },
      { Effect = "Allow"; Action = ["ssm:GetParameter"]; Resource = "arn:aws:ssm:${var.aws_region}:*:parameter/shopcloud/prod/*" },
      { Effect = "Allow"; Action = ["kms:Decrypt"]; Resource = module.kms.key_arn }
    ]
  })
...
module "irsa_auth" {
  ...
  inline_policy     = jsonencode({
    ...
    Statement = [
      ...
      { Effect = "Allow"; Action = ["secretsmanager:GetSecretValue"]; Resource = "arn:aws:secretsmanager:${var.aws_region}:*:secret:shopcloud/prod/auth*" },
      { Effect = "Allow"; Action = ["kms:Decrypt"]; Resource = module.kms.key_arn }
    ]
  })
  tags = local.common_tags
...
module "irsa_admin" {
  ...
  inline_policy     = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Effect = "Allow"; Action = ["secretsmanager:GetSecretValue"]; Resource = "arn:aws:secretsmanager:${var.aws_region}:*:secret:shopcloud/prod/admin*" },
