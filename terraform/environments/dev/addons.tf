# IRSA roles for cluster add-ons — dev cluster.
# Same structure as prod but scoped to shopcloud/dev/* secrets.
    ...
    Statement = [
      ...
      },
      {
        ...
        Action = [
          ...
          "elasticloadbalancing:RemoveTags", "elasticloadbalancing:CreateLoadBalancer",
          "elasticloadbalancing:CreateTargetGroup", "elasticloadbalancing:DeleteLoadBalancer",
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:secretsmanager:${var.aws_region}:*:secret:shopcloud/dev/*"
      },
      {
        Effect   = "Allow"
        Action   = ["kms:Decrypt"]
